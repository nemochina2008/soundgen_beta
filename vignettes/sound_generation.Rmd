...
```{r}
library(soundgen)
s = soundgen(formants = 'uai', repeatBout = 1, nSyl = 3, play = TRUE)
...
```{r}
s = soundgen(formants = 'uai', repeatBout = 3, nSyl = 1, play = TRUE)
# playme(s)
```
...
```{r fig.width = 4, fig.height = 4}
a = getSmoothContour(anchors = data.frame(time = c(0, .5, 1), 
                                          value = c(350, 700, 350)),
  len = 7000, thisIsPitch = TRUE, plot = TRUE, samplingRate = 3500)
```
...
Rapid amplitude modulation imitating a trill is implemented by multiplying the synthesized waveform by a sine wave with frequency `trillFreq` and amplitude `trillDep`:
```{r fig.width = 5, fig.height = 5}
...
```{r}
s = soundgen(formants = 'ai', play = TRUE)
s = soundgen(formants = 'aaai', play = TRUE)
```
...
### Vibrato
Technically also a pitch effect, vibrato adds a regular (in contrast to irregular jitter and temperature-related random drift), perfectly sinusoidal frequency modulation to F0 contour:
```{r}
