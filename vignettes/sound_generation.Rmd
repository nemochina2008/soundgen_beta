...
```{r}
library(soundgen)
s = soundgen(play = TRUE)  # default sound: a short [a] by a male speaker
...
Audio playback may fail, depending on your platform and installed software. Soundgen relies on `tuneR` library for audio playback. If `playme` or `soundgen(play = TRUE)` throw an error, save some sound as a vector first: `sound = soundgen(play = FALSE)`. Then try to find a way to play this vector. See the <a href="https://cran.r-project.org/web/packages/seewave/vignettes/seewave_IO.pdf">seewave vignette</a> for an in-depth discussion of audio playback in R. Some tips are also available <a href="http://music.informatics.indiana.edu/courses/I546/tuneR_play.pdf"here</a>.

The basic workflow in the Shiny app is as follows:

1. Start the app by typing `soundgen_app()`. If RStudio doesn't open it in a browser by default, click "Open in browser". Firefox and Chrome are known to work. Safari will probably fail to play back the generated audio, although it can still be exported as a .wav file.
...
```{r}
sound = soundgen(pitchAnchors = 440, play = TRUE)  # steady pitch at 440 Hz
sound = soundgen(pitchAnchors = 3000:2000, play = TRUE)  # downward chirp
sound = soundgen(pitchAnchors = c(150, 250, 100), sylLen = 700, play = TRUE)  # up and down
```
...
sound = soundgen(pitchAnchors = anchors, sylLen = 1000, play = TRUE)
```
...
s = soundgen(sylLen = 700, play = TRUE,
...
par(mfrow = c(1, 2))
sound1 = sin(2 * pi * 1:5000 * 100 / 16000) # pure tone, 300 Hz
sound2 = sin(2 * pi * 1:5000 * 200 / 16000) # pure tone, 380 Hz
...
sound1 = soundgen(sylLen = 700, pitchAnchors = 250:180, formants = 'aaao', 
                  addSilence = 100, play = TRUE)
sound2 = soundgen(nSyl = 2, sylLen = 150, pitchAnchors = 4300:2200, 
                  temperature = 0, addSilence = 0, play = TRUE)
...
playme(comb)
spectrogram(comb, 16000, windowLength = 15, ylim = c(0, 5))
```
