server = function(input, output, session) {
  ...
  durSyl_withBreathing = reactive({
    ifelse(!sum(myPars$breathingAnchors$value>throwaway_dB)>0, input$sylDur_mean, min(0,myPars$breathingAnchors$time[1])+max(input$sylDur_mean,myPars$breathingAnchors$time[length(myPars$breathingAnchors$time)])) # the duration of a single syllable with breathing
  })
  ...
  reset_all = reactive({  # this key function is EXTREMELY bug-prone - careful with what you change! The right order is crucial
    ...
    myPars[['pitchAnchors_global']] = data.frame('time'=c(0,1),'value'=c(0,0))
    myPars[['mouthAnchors']] = data.frame('time'=c(0,1),'value'=c(0.5,0.5))
    myPars[['amplAnchors_global']] = data.frame('time'=c(0,1),'value'=c(-throwaway_dB,-throwaway_dB))
    myPars[['amplAnchors']] = data.frame('time'=c(0,1),'value'=c(-throwaway_dB,-throwaway_dB))
    ...
    if (!is.null(preset$pitchAnchors)){
      updateSliderInput(session, 'pitchRange', value=c( round(min(preset$pitchAnchors$value)/2^(1/12),0), round(max(preset$pitchAnchors$value)*2^(1/12),0) ) )
    }
  ...
  observeEvent(input$plot_intonation_click, {
    ...
    if (delta_x < 0.05){
      ...
      myPars[['pitchAnchors']] = data.frame ('time'=c(myPars$pitchAnchors$time, click_x),
                                             'value'=c(myPars$pitchAnchors$value, click_y)) # convoluted, but otherwise problems with unwanted dataframe-list conversion, etc
    }
  ...
  observeEvent(input$plot_intonation_dblclick, {
    ...
    closestPoint = nearPoints(ref, input$plot_intonation_dblclick, xvar='time', yvar='value', threshold=100000, maxpoints=1)
  ...
  observeEvent(input$pitch_flatten, {
    myPars[['pitchAnchors']] = data.frame('time'=c(0,1),
                                          'value'=rep(myPars$pitchAnchors$value[1],2))  # flat pitch equal to the first pitch anchor
  })

  output$pitch_anchors = renderTable(expr=data.frame('Time, ms'=round(myPars$pitchAnchors$time*input$sylDur_mean,0), 'Pitch, Hz'=round(myPars$pitchAnchors$value,0), row.names=1:length(myPars$pitchAnchors$time)), digits=0, align='c', rownames=FALSE)
  ...
  myPitchContour_global <- reactive({
    if (input$nSyl>1){
      getDiscreteContour (anchors=myPars$pitchAnchors_global, len=input$nSyl, method='spline', plot=TRUE, ylab='Pitch delta, semitones', value_floor=permittedValues['pitchDeltas','low'], value_ceiling=permittedValues['pitchDeltas','high'], ylim=c(permittedValues['pitchDeltas','low'], permittedValues['pitchDeltas','high']))
    } else {
  ...
  observeEvent(input$plot_intonation_click_global, {
    ...
    if (delta_x < 0.2){
      ...
      myPars[['pitchAnchors_global']] = data.frame ('time'=c(myPars$pitchAnchors_global$time, click_x),
                                                    'value'=c(myPars$pitchAnchors_global$value, click_y)) # convoluted, but otherwise problems with unwanted dataframe-list conversion, etc
    }
    # sort the updated dataframe of pitch anchors to make sure the point are in the right order (otherwise it's hard to keep track of which are the first and last anchors - and we have to, since those cannot be removed)
  ...
  observeEvent(input$plot_intonation_dblclick_global, {
    ...
    closestPoint = nearPoints(ref, input$plot_intonation_dblclick_global, xvar='time', yvar='value', threshold=100000, maxpoints=1)
    idx = as.numeric(rownames(closestPoint))
  ...
  observeEvent(input$pitch_flatten_global, {
    myPars[['pitchAnchors_global']] = data.frame('time'=c(0,1),'value'=c(0,0))  # flat pitch modulation across syllables
  })

  output$pitch_anchors_global = renderTable(expr=data.frame('Time 0 to 1'=myPars$pitchAnchors_global$time, 'Pitch delta, semitones'=round(myPars$pitchAnchors_global$value,0), row.names=1:length(myPars$pitchAnchors_global$time)), digits=2, align='c', rownames=FALSE)
  ...
  myBreathingContour <- reactive({
    ...
    getSmoothContour(anchors=myPars$breathingAnchors, plot=TRUE, xlim=c(br_xlim_low, br_xlim_high), ylim=c(br_ylim_low, br_ylim_high), voiced=input$sylDur_mean, contourLabel='breathing', value_floor=br_ylim_low, value_ceiling=br_ylim_high, yaxp=c(br_ylim_low, br_ylim_high, nTicks))
  })
  ...
  observeEvent(input$plot_unvoiced_click, {
    ...
    if (delta_x < 0.05*durSyl_withBreathing()){
      ...
      myPars[['breathingAnchors']] = data.frame ('time'=c(myPars$breathingAnchors$time, click_x),
                                                 'value'=c(myPars$breathingAnchors$value, click_y)) # convoluted, but otherwise problems with unwanted dataframe-list conversion, etc
    }
    # sort the updated dataframe of pitch anchors to make sure the point are in the right order (otherwise it's hard to keep track of which are the first and last anchors - and we have to, since those cannot be removed)
  ...
  observeEvent(input$plot_unvoiced_dblclick, {
    closestPoint = nearPoints(as.data.frame(myPars[['breathingAnchors']]), input$plot_unvoiced_dblclick, xvar='time', yvar='value', threshold=100000, maxpoints=1)
    idx = as.numeric(rownames(closestPoint))
  ...
  observeEvent(input$breathing_flatten, {
    myPars[['breathingAnchors']] = data.frame(
      ...
      'value'=rep(myPars$breathingAnchors$value[1],2)  # flat pitch equal to the first pitch anchor
    )})

  output$breathing_anchors = renderTable(expr=data.frame('Time, ms'=round(myPars$breathingAnchors$time,0), 'Amplitude, dB'=round(myPars$breathingAnchors$value,0), row.names=1:length(myPars$breathingAnchors$time)), digits=0, align='c', rownames=FALSE)
  ...
  myMouthOpening <- reactive({
    getSmoothContour(anchors=myPars$mouthAnchors, len=durSyl_withBreathing()/1000*1000, samplingRate=1000, plot=TRUE, contourLabel='mouth', xlim=c(0,durSyl_withBreathing()), xaxs="i", ylim=c(permittedValues['mouthOpening','low'],permittedValues['mouthOpening','high']), value_floor=permittedValues['mouthOpening','low'], value_ceiling=permittedValues['mouthOpening','high'])  # # xaxs="i" to enforce exact axis limits, otherwise we exceed the range. OR: xlim=range(myPars$breathingAnchors$time)
  })
  ...
  observeEvent(input$plot_mouth_click, {
    ...
    if (delta_x < 0.05){
      ...
      myPars[['mouthAnchors']] = data.frame ('time'=c(myPars$mouthAnchors$time, click_x),
                                             'value'=c(myPars$mouthAnchors$value, click_y)) # convoluted, but otherwise problems with unwanted dataframe-list conversion, etc
    }
    # sort the updated dataframe of pitch anchors to make sure the point are in the right order (otherwise it's hard to keep track of which are the first and last anchors - and we have to, since those cannot be removed)
  ...
  observeEvent(input$plot_mouth_dblclick, {
    ...
    closestPoint = nearPoints(ref, input$plot_mouth_dblclick, xvar='time', yvar='value', threshold=100000, maxpoints=1)
    idx = as.numeric(rownames(closestPoint))
  ...
  output$mouth_anchors = renderTable(expr=data.frame('Time, ms'=as.integer(round(myPars$mouthAnchors$time*durSyl_withBreathing())), 'Open'=myPars$mouthAnchors$value, row.names=1:length(myPars$mouthAnchors$time)), digits=2, align='c', rownames=FALSE)
  ...
  amplEnvelope_syl <- reactive({
    getSmoothContour (anchors=myPars$amplAnchors, plot=TRUE, xaxs="i", xlim=c(0,input$sylDur_mean), ylim=c(0, -throwaway_dB), value_floor=0, value_ceiling=-throwaway_dB, len=input$sylDur_mean/1000*1000, samplingRate=1000)  # xaxs="i" to enforce exact axis limits, otherwise we exceed the range
  })
  ...
  observeEvent(input$plot_ampl_syl_click, {
    ...
    if (delta_x < 0.05){
      ...
      myPars[['amplAnchors']] = data.frame ('time'=c(myPars$amplAnchors$time, click_x),
                                            'value'=c(myPars$amplAnchors$value, click_y)) # convoluted, but otherwise problems with unwanted dataframe-list conversion, etc
    }
  ...
  observeEvent(input$plot_ampl_syl_dblclick, {
    ...
    ref$time = ref$time*input$sylDur_mean
    closestPoint = nearPoints(ref, input$plot_ampl_syl_dblclick, xvar='time', yvar='value', threshold=100000, maxpoints=1)
    idx = as.numeric(rownames(closestPoint))
  ...
  observeEvent(input$ampl_syl_flatten, {
    myPars[['amplAnchors']] = data.frame('time'=c(0,1),
                                         'value'=rep(myPars$amplAnchors$value[1],2))  # flat ampl equal to the first ampl anchor
  })

  output$ampl_syl_anchors = renderTable(expr=data.frame('Time, ms'=as.integer(round(myPars$amplAnchors$time*input$sylDur_mean,0)), 'Amplitude'=myPars$amplAnchors$value, row.names=1:length(myPars$amplAnchors$time)), digits=0, align='c', rownames=FALSE)
  ...
  amplEnvelope_global <- reactive({
    if (input$nSyl>1){
      getSmoothContour (anchors=myPars$amplAnchors_global, xaxs="i", xlim=c(0,durTotal()), ylim=c(0, -throwaway_dB), value_floor=0, value_ceiling=-throwaway_dB, len=durTotal()/1000*100, samplingRate=100, plot=TRUE)
    } else {
      plot (1:10, 1:10, type='n', xlab='', ylab='', axes=FALSE)
      text (x=5, y=5, labels = 'Need >1 syllable!', adj=.5, col='blue', cex=1)
    }
  ...
  observeEvent(input$plot_ampl_global_click, {
    ...
    if (delta_x < 0.05){
      ...
      myPars[['amplAnchors_global']] = data.frame ('time'=c(myPars$amplAnchors_global$time, click_x),
                                                   'value'=c(myPars$amplAnchors_global$value, click_y)) # convoluted, but otherwise problems with unwanted dataframe-list conversion, etc
    }
    # sort the updated dataframe of anchors to make sure the point are in the right order (otherwise it's hard to keep track of which are the first and last anchors - and we have to, since those cannot be removed)
  ...
  observeEvent(input$plot_ampl_global_dblclick, {
    ...
    closestPoint = nearPoints(ref, input$plot_ampl_global_dblclick, xvar='time', yvar='value', threshold=100000, maxpoints=1)
    idx = as.numeric(rownames(closestPoint))
  ...
  observeEvent(input$ampl_global_flatten, {
    myPars[['amplAnchors_global']] = data.frame('time'=c(0,1),
                                                'value'=rep(myPars$amplAnchors_global$value[1],2))  # flat ampl equal to the first ampl anchor
  })

  output$ampl_global_anchors = renderTable(expr=data.frame('Time, ms'=as.integer(round(myPars$amplAnchors_global$time*durTotal(),0)), 'Amplitude'=myPars$amplAnchors_global$value, row.names=1:length(myPars$amplAnchors_global$time)), digits=0, align='c', rownames=FALSE)
  ...
  output$plot_spectrum = renderPlot({
    # seewave::meanspec (myPars$sound, f=input$samplingRate, dB='max0', wl=floor(input$spec_windowLength*input$samplingRate/1000/2)*2, flim=c(0,10), main='Spectrum')
  ...
  mycall = reactive({
    paste0('generateBout (nSyl=', input$nSyl,', sylDur_mean=',input$sylDur_mean, ', pauseDur_mean=', input$pauseDur_mean, ', noiseAmount=',input$noiseAmount, ', noiseIntensity=', input$noiseIntensity, ', attackLen=',input$attackLen, ', jitterDep=',input$jitterDep, ', jitterLength_ms=',input$jitterLength_ms, ', vibratoFreq=',input$vibratoFreq, ', vibratoDep=',input$vibratoDep, ', shimmerDep=',input$shimmerDep, ', formantStrength=',input$formantStrength, ', extraFormants_ampl=',input$extraFormants_ampl, ', creakyBreathy=',input$creakyBreathy, ', rolloff_exp=',input$rolloff_exp, ', rolloff_exp_delta=',input$rolloff_exp_delta, ', adjust_rolloff_per_kHz=',input$adjust_rolloff_per_kHz, ', quadratic_delta=',input$quadratic_delta, ', quadratic_nHarm=',input$quadratic_nHarm, ', rolloff_lip=',input$rolloff_lip, ', trill_dep=',input$trill_dep, ', trill_freq=',input$trill_freq, ', rolloff_breathing=',input$rolloff_breathing, ', temperature=',input$temperature, ', min_epoch_length_ms=',input$min_epoch_length_ms, ', g0=',input$g0, ', sideband_width_hz=',input$sideband_width_hz,  ', maleFemale=',input$maleFemale, ', samplingRate=',input$samplingRate, ', windowLength_points=',windowLength_points, ', overlap=',overlap, ', pitch_floor=',input$pitchFloorCeiling[1], ', pitch_ceiling=',input$pitchFloorCeiling[2], ', pitchSamplingRate=',input$pitchSamplingRate, ', vocalTract_length=',input$vocalTract_length, ', repeatBout=',input$repeatBout,
           ...
           '), value=c(',paste0(myPars$pitchAnchors$value,collapse=","),
