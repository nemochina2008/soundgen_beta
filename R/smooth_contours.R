...
#'   voiced=200, ampl_floor=0, plot=TRUE, main='', samplingRate=16000) # breathing
#' a=soundgen:::getSmoothContour (anchors=data.frame(time=c(0,.1), ampl=c(350, 800)),
#'   len=5500, thisIsPitch=TRUE, plot=TRUE, samplingRate=3500) # pitch
#'
...
#'   len=500, thisIsPitch=TRUE, plot=TRUE, samplingRate=500)
getSmoothContour = function(anchors=data.frame(time=c(0,1), ampl=c(0,1)), len=NULL, thisIsPitch=FALSE, method=c('spline','loess')[2], ampl_floor=NULL, ampl_ceiling=NULL, plot=FALSE, main='', xlim=NULL, ylim=NULL, samplingRate=44100, voiced=NULL, contourLabel=NULL, ...){
  if (!is.null(ampl_floor)){anchors$ampl[anchors$ampl<ampl_floor] = ampl_floor}
  if (!is.null(ampl_ceiling)){anchors$ampl[anchors$ampl>ampl_ceiling] = ampl_ceiling}
  if (thisIsPitch){
    anchors$ampl = HzToSemitones(anchors$ampl)
    if (!is.null(ampl_floor)) ampl_floor = HzToSemitones(ampl_floor)
    if (!is.null(ampl_ceiling)) ampl_ceiling = HzToSemitones(ampl_ceiling)
  }

  if (is.null(len)){ # if len is null, we expect that anchors$time encoded the desired duration, in ms
  ...
  if (!is.numeric(duration_ms)) {return (NA)}

  time = 1:len
  if (nrow(anchors)<1){
    stop ('getSmoothContour() requires at least one anchor') # smoothContour = rep(0, length(time))
  } else if (nrow(anchors)==1){ # flat
    smoothContour = rep(anchors$ampl[1], len)
  } else if (nrow(anchors)==2){ # linear
    smoothContour = seq(anchors$ampl[1], anchors$ampl[2], length.out=len)
  } else { # smooth contour
    if (method=='spline'){
      smoothContour = spline(anchors$ampl, n=len, x=anchors$time)$y  # plot(smoothContour, type='l')
    } else if (method=='loess'){
      anchor_time_points = anchors$time - min(anchors$time)
      anchor_time_points = anchor_time_points/max(anchor_time_points)*len # floor(anchors$time*samplingRate/1000)
  ...
  if (plot){
    idx = seq(1, len, length.out=min(len,100)) # for plotting, shorten smoothContour to max 100 points to reduce processing load
    smoothContour_downsampled = smoothContour[idx]

    if (thisIsPitch){
      # pitch - log-transformed
      if (is.null(ylim) || (min(smoothContour_downsampled)<HzToSemitones(ylim[1]) | max(smoothContour_downsampled)>HzToSemitones(ylim[2]))){
        ylim=round ( c(min(smoothContour_downsampled)/1.1, max(smoothContour_downsampled)*1.1) )
      } else {
        ylim = HzToSemitones(ylim)
      }
      lbls_semitones = unique ( seq(ylim[1],ylim[2],length.out=5) ) # unique to remove duplicates, max 5 labels
      ...
      plot(time[idx]/samplingRate*1000, smoothContour_downsampled, type='l', yaxt = "n", ylab='Frequency, Hz', xlab='Time, ms', main='Pitch contour', ylim=ylim, ...) # ylim=c(25,max(73, max(pitch_semitones)*1.1))
      ...
      if (!max(anchors$time)>1){anchors$time = anchors$time * duration_ms} # presuming that len was specified and anchors$time are on a relative scale, we transform to ms for plotting
      par(mar=c(5,4,4,3)) # c(bottom, left, top, right)
      if(is.null(xlim)) xlim = c( min(0,anchors$time[1]), anchors$time[length(anchors$time)] )
      if(is.null(ylim)) ylim = c( min(0,min(anchors$ampl[1])), max(0,max(anchors$ampl)) )
      x = seq(anchors$time[1], anchors$time[length(anchors$time)], length.out=length(smoothContour_downsampled))
      plot(x=x, y=smoothContour_downsampled, type='l', ylab='Amplitude', xlab='Time, ms', xlim=xlim, ylim=ylim, ...)
      points(anchors$time, anchors$ampl, col='blue', cex=3)
      if (is.numeric(voiced)){
        lines(x=c(0,voiced), y=c(0,0), col='blue', lwd=10)
        text(x=voiced/2, y=abs(ylim[2]-ylim[1])/25, adj=0.5, labels='voiced part', col='blue')
        text(x=anchors$time[nrow(anchors)], y=anchors$ampl[nrow(anchors)]-(ylim[2]-ylim[1])/25, adj=1, labels=contourLabel, col='red')
      }
      par(mar=c(5,4,4,2))
    }
  }

  smoothContour[is.na(smoothContour)] = 0  # NA's may arise if the first anchor time > 0
  if (thisIsPitch) {smoothContour = semitonesToHz(smoothContour)}
...
#' soundgen:::getDiscreteContour (len=10, method='spline', plot=TRUE, ylab='Semitones',
#'   anchors=data.frame(time=c(0,.2,.6,1),ampl=c(0,-3,1,0)))
getDiscreteContour = function(len, anchors=data.frame(time=c(0,1),ampl=c(1,1)), method=c('spline','loess')[2], ampl_floor=NULL, ampl_ceiling=NULL, ylim=NULL, plot=FALSE, ...){
  contour = getSmoothContour(len=len, anchors=anchors, method=method, ampl_floor=ampl_floor, ampl_ceiling=ampl_ceiling)
  if (plot){
    if(is.null(ylim)) {ylim = c( min(min(contour),min(anchors$ampl)), max(max(contour),max(anchors$ampl)))}
    plot (contour, type='b', xlab='Syllable', col='red', bg='red', cex=1, pch=23, ylim=ylim, ...)
    points (x=anchors$time*(len-1)+1, y=anchors$ampl, col='blue', cex=3)
  }
  return(contour)
}
...
#'   temperature=.2, temp_coef=.1, low=c(0,50), high=c(1,1000), wiggleAllRows = FALSE) # pitch
...
wiggleAnchors = function(df, temperature, temp_coef, low, high, wiggleAllRows=FALSE){
  ...
  if(wiggleAllRows){
    ...
    if (!is.na(idx1)){
      temp = df[idx1,1]
      df[idx1,1] =  rnorm_bounded(n=length(temp), mean=temp, sd=temperature*max(abs(df[,1]))*temp_coef, low=low[1], high=high[1], roundToInteger=FALSE)
    }
  }
  df[,2] = rnorm_bounded(n=nrow(df), mean=df[,2], sd=temperature*abs(high[2]-low[2])*temp_coef, low=low[2], high=high[2], roundToInteger=FALSE)
  return(df)
}
