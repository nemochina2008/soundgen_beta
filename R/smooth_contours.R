...
#'   len = 500, thisIsPitch = TRUE, plot = TRUE, samplingRate = 500)
getSmoothContour = function(anchors = data.frame(
  time = c(0, 1), ampl = c(0, 1)), len = NULL,
  thisIsPitch = FALSE, method = c('spline', 'loess')[2],
  ampl_floor = NULL, ampl_ceiling = NULL, plot = FALSE,
  main = '', xlim = NULL, ylim = NULL, samplingRate = 44100,
  voiced = NULL, contourLabel = NULL, ...) {
  ...
  if (nrow(anchors) < 1){
    ...
    if (method == 'spline'){
      ...
      # for long duration etc, larger span may be necessary
      # to avoid an error in loess
  ...
  if (plot){
    ...
    if (thisIsPitch){
      ...
      if (is.null(ylim) || (
        ...
        ylim = round(c(min(smoothContour_downsampled) / 1.1,
      ...
      x = seq(anchors$time[1], anchors$time[length(anchors$time)],
...
#'   ampl = c(0, -3, 1, 0)))
getDiscreteContour = function(len, anchors = data.frame(
  time = c(0, 1), ampl = c(1, 1)),
  method = c('spline', 'loess')[2], ampl_floor = NULL, ampl_ceiling = NULL,
  ylim = NULL, plot = FALSE, ...) {
  contour = getSmoothContour(len = len, anchors = anchors, method = method,
    ampl_floor = ampl_floor, ampl_ceiling = ampl_ceiling)
  if (plot){
    if(is.null(ylim)) {
    ...
    points (x = anchors$time * (len - 1) + 1, y = anchors$ampl,
            col = 'blue', cex = 3)
  }
  return(contour)
}
...
wiggleAnchors = function(df, temperature, temp_coef, low, high,
                         wiggleAllRows = FALSE) {
  if (sum(is.na(df)) > 0) return(NA)
  if(wiggleAllRows) {
    df[, 1] = rnorm_bounded(n = nrow(df), mean = df[, 1],
              sd = temperature * max(abs(df[, 1])) * temp_coef,
              low = low[1], high = high[1], roundToInteger = FALSE)
  } else { # don't wiggle the first and last time values
    idx1 = ifelse(nrow(df) < 3, NA, 2:(nrow(df) - 1))
    if (!is.na(idx1)){
      temp = df[idx1, 1]
      df[idx1, 1] =  rnorm_bounded(n = length(temp), mean = temp,
                     sd = temperature * max(abs(df[, 1])) * temp_coef,
                     low = low[1], high = high[1], roundToInteger = FALSE)
    }
  }
  df[, 2] = rnorm_bounded(n = nrow(df), mean = df[,2],
            sd = temperature * abs(high[2] - low[2]) * temp_coef,
            low = low[2], high = high[2], roundToInteger = FALSE)
  return(df)
}
