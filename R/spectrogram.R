...
getFrameBank = function(sound, samplingRate, windowLength_points, wn, step, zp, filter=NULL){
  ...
  sound = sound / max( abs(max(sound)), abs(min(sound))) # normalize to range from no less than -1 to no more than +1

  duration = length(sound) / samplingRate
  myseq = seq(1, max(1, (length(sound)-windowLength_points)), step/1000*samplingRate)
  if (is.null(filter)) {
    filter=ftwindow_modif(wl=windowLength_points, wn=wn)
  }
  zpExtra = floor( (zp-windowLength_points)/2) * 2 # how many extra zeroes we pad with. Made even
  if (zpExtra>0){
    frameBank = apply (as.matrix(myseq), 1, function(x) c(rep(0,zpExtra/2), sound[x:(windowLength_points + x - 1)]*filter, rep(0,zpExtra/2)) )
  } else {
    frameBank = apply (as.matrix(myseq), 1, function(x) sound[x:(windowLength_points + x - 1)]*filter )
  }
  return (frameBank)
}
...
#' @return Returns entropy of a vector on a scale 0 to 1 (i.e., normalized so
#'   that entropy does not depend on the length of input vector). Zeroes are
#'   transformed into 1e-10, negative values produce NaN.
getEntropy = function(x) {
  # Needed only for UI (spectrogram), not needed for the command-line generator. Returns entropy of a vector on a scale 0 to 1
  x = ifelse (x==0, 1e-10, x)  # otherwise log0 gives NaN
  return ( -sum(log2(x/sum(x))*x/sum(x)) / log(length(x))*log(2) )
}
...
spectro_denoised = function (soundfile, frameBank=NULL, samplingRate=NULL, windowLength=50, step=15, wn='gaussian', zp=0, median_smoothing_freq=0, median_smoothing_time=0, denoise_median_time=0, percentNoise=0, noiseReduction=0, contrast=.2, brightness=0, method=c('spectrum','spectralDerivative')[1], output=c('none','original','processed')[1], ylim=NULL, plot=T, osc=F, colorTheme = c('bw','seewave','...')[1], xlab='', ...) {
  ...
  if (class(soundfile)=='character'){
    ...
    duration = length(sound) / samplingRate
    frameBank = getFrameBank (sound=sound, samplingRate=samplingRate, windowLength_points=windowLength_points, step=step, zp=zp, wn=wn, filter=NULL)
    ...
    if (is.null(samplingRate)){
      ...
      duration = length(sound) / samplingRate
      windowLength_points = floor(windowLength/1000 * samplingRate / 2) * 2
      frameBank = getFrameBank (sound=soundfile, samplingRate=samplingRate, windowLength_points=windowLength_points, step=step, zp=zp, wn=wn, filter=NULL)
    }
  }
  if (class(frameBank)!='matrix') {
    stop('Input format not recognized. Please provide path to .wav file, a vector of amplitudes plus samplingRate, or a preprocessed frameBank')
  }

  # FFT
  windowLength_points = floor(windowLength/1000 * samplingRate / 2) * 2
  z <- apply(frameBank, 2, function(x) fft(x)[1:(floor(nrow(frameBank)/2))] )  # fft of each frame
  X <- seq(0, duration*1000, length.out=ncol(z)) # time stamp
  Y <- seq(0, (samplingRate/2) - (samplingRate/windowLength_points), length.out=nrow(z))/1000 # frequency stamp
  rownames(z) = Y; colnames(z)=X
  Z = t(abs(z))

  if (method=='spectralDerivative'){
    dZ_dt = cbind ( rep(0,nrow(Z)), t(apply (Z, 1, diff)) ) # first derivative of spectrum by time
    dZ_df = rbind ( rep(0,ncol(Z)), apply (Z, 2, diff) ) # first derivative of spectrum by frequency
    Z1 = sqrt(dZ_dt^2 + dZ_df^2) # length of gradient vector
  } else {
    Z1 = Z # this is our raw spectrogram
  }

  # removing noise. NB: the order of these operations is crucial, don't change it!
  if(median_smoothing_time>1) {
    Z1 = t(apply(Z1, 1, function(x)zoo::rollmedian(x,k=median_smoothing_time,fill=0))) # time domain
  }
  if (median_smoothing_freq>1) {
    Z1 = apply(Z1, 2, function(x)zoo::rollmedian(x,k=median_smoothing_freq,fill=0)) # freq domain
  }
  if (denoise_median_time > 0){
    Z1 = t (apply(Z1, 1, function(x)x - quantile(x, probs = denoise_median_time))) # for each freq bin, subtract median or another quantile
  }
  ...
  if (percentNoise>0){ # silence frames with entropy above threshold
    entr = apply(Z1, 1, function(x)getEntropy(x))
    q = quantile(entr, probs=1-percentNoise) # plot(entr, type='l'); lines(x=1:length(entr),y=rep(q,length(entr)), col='blue', lty=2)
    idx = as.numeric(which(entr>q))
    noise_spectrum = as.numeric(apply (Z1[idx,], 2, mean)) # plot(noise_spectrum, type='l')
  ...
  if (plot){
    ...
    seewave::filled.contour.modif2 (x=X, y=Y, z=Z1, levels=seq(0, 1, length=30), color.palette=color.palette, ylim=ylim, ...) # , xlab='Time, ms', ylab = 'Frequency, kHz'
    on.exit(par(op))
    layout(matrix(c(1, 1)))
  }
