...
#'   range 0 to 100)
#' @param pitchEffectsAmount a number between 0 to 100: 0 = returns all zeroes;
...
getIntegerRandomWalk = function(rw,
                                ...
                                minLength = 50) {
  len = length(rw)
  if (pitchEffectsAmount == 0) return(rep(0, len))
  if (pitchEffectsAmount == 100) return(rep(2, len))

  # calculate thresholds for different noise regimes
  q1 = noiseThresholdsDict$q1[pitchEffectsAmount + 1]
  # +1 b/c the rows indices in noiseThresholdsDict start from 0, not 1
  q2 = noiseThresholdsDict$q2[pitchEffectsAmount + 1]
